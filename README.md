...
- 优化消息 Reaction 展示：在表情右侧追加触发用户标签，支持快速查看是谁做了反馈。
- 新增 `Reaction__users` 样式，控制最大宽度、单行省略与可读性，避免挤压原有互动区。
- 对英文短词表情名做大写格式化（如 `ok` -> `OK`），提升统一性。
- 支持 `+N` 聚合展示，触发用户过多时仍可保持紧凑显示。

### 补充说明（第二次提交）

- 调整 Reaction 用户标签布局与样式细节，减少长用户名对主消息区的干扰。
- 保留原有表情交互逻辑，仅增强显示层，降低改造引入行为回归的风险。
- 为后续接入 OpenClaw 的任务状态反馈（如已读/接单类反应）预留更清晰的可视化承载位。
