...
- 在频道头部菜单新增“清空频道历史”操作入口，并补充对应前端测试，支持批量清理历史消息。
- 新增“永久删除频道”菜单项与测试用例，覆盖菜单展示、权限判断与动作触发流程。
- 调整 `channel.ts` 与频道菜单逻辑，确保危险操作入口在公有/私有频道场景下行为一致。

## 本次改进点（2026-03-16）

- 频道治理功能补齐：对“清空历史/永久删除”两类高风险操作继续打磨交互与测试用例，提升操作确定性。
- 优化频道头部菜单可见性规则：确保不同频道类型与权限场景下入口展示一致且可预期。
- 稳定前端动作链路：同步收敛 `channel.ts` 与菜单动作实现，降低回归风险。
