...
export default class AtMentionProvider extends Provider {
    ...
    specialMentions() {
        ...
        const specialMentions: SpecialMention[] = [
            {username: 'here'},
            {username: 'channel'},
            {username: 'all', displayName: '所有人', mentionTerm: '所有人'},
        ];

        const latestPrefixLower = this.latestPrefix.toLowerCase();
        return specialMentions.filter((item) =>
            item.username.toLowerCase().startsWith(latestPrefixLower) ||
            item.displayName?.toLowerCase().startsWith(latestPrefixLower),
        );
    }
...
export function membersGroup(items: CreatedProfile[]) {
    return {
        ...
        items,
        terms: items.map((profile) => buildDisplayMentionTerm(profile)),
...
export function defaultAgentGroup(item: CreatedProfile) {
    return {
        key: 'defaultAgent',
        items: [item],
        terms: [buildDisplayMentionTerm(item)],
        component: AtMentionSuggestion,
    };
}
...
export function specialMentionsGroup(items: SpecialMention[]) {
    return {
        key: 'specialMentions',
        label: defineMessage({id: 'suggestion.mention.special', defaultMessage: 'Special Mentions'}),
        items,
        terms: items.map((item) => '@' + (item.mentionTerm || item.displayName || item.username)),
        component: AtMentionSuggestion,
    };
}
...
export function nonMembersGroup(items: CreatedProfile[]) {
    return {
        ...
        items,
        terms: items.map((item) => buildDisplayMentionTerm(item)),
        component: AtMentionSuggestion,
    };
}

function buildDisplayMentionTerm(profile: UserProfile) {
    return '@' + displayUsername(profile, General.TEAMMATE_NAME_DISPLAY.SHOW_NICKNAME_FULLNAME);
}
