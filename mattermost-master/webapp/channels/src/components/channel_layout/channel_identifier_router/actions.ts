...
export function onChannelByIdentifierEnter({match, history}: MatchAndHistory): ActionFuncAsync {
    return async (dispatch, getState) => {
        ...
        if (path === 'channels' && isDirectChannelIdentifier(identifier)) {
            const currentUserId = getCurrentUserId(state);
            if (currentUserId) {
                const otherUserId = Utils.getUserIdFromChannelId(identifier.toLowerCase(), currentUserId);
                history.replace(`/${team}/messages/${otherUserId}`);
                return {data: undefined};
            }
        }

        let teamObj = getTeamByName(state, team);
        if (!teamObj) {
            const teamResult = await dispatch(getTeamByNameAction(team));
            if ('error' in teamResult) {
                return {data: undefined};
            }
            teamObj = teamResult.data;
        }
