...
export function fetchTeamScheduledPosts(teamId: string, includeDirectChannels: boolean, prune?: boolean) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        try {
            ...
            if ((error as {id?: string}).id === 'api.scheduled_posts.license_error') {
                const scheduledPostsByTeamId: {[key: string]: ScheduledPost[]} = {
                    [teamId]: [],
                };
                if (includeDirectChannels) {
                    scheduledPostsByTeamId.directChannels = [];
                }
                ...
                return {data: {data: scheduledPostsByTeamId}};
            }
